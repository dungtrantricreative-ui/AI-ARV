...
- **"AI trưởng nhóm" (director)**: tự quyết định đoạn nào chỉ cần đọc transcript (rẻ, nhanh) và
  ...
  chi phí khi gặp phim hành động ít thoại.
...
- **Render 3 giai đoạn tối ưu tốc độ**: cắt song song đa luồng (tự tận dụng CPU, tự dùng GPU NVIDIA
  qua NVENC nếu máy có) → ghép bằng concat demuxer (không re-encode, gần như tức thời) → trộn
  audio/phụ đề/nhạc nền. Log tiến trình % + ETA theo thời gian thực, có màu để dễ theo dõi.
...
> Các subcommand `setup`, `download`, `prepare` hiện được khai báo trong `main.py` nhưng **chưa
> được cài đặt logic** (no-op) — dùng chế độ Agent hoặc `render` như trên là cách hoạt động chính
> thức hiện tại.
...
sync_assemble.py          Ghép video + audio + phụ đề + nhạc nền thành file cuối (3 giai đoạn)
modal_app.py               (Tuỳ chọn) khung chạy trên Modal.com cloud, chưa implement pipeline đầy đủ
